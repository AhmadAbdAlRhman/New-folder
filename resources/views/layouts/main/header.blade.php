      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
         ...
         <ul class="navbar-nav align-items-center ml-md-auto">
            ...
            <li style="margin-left:auto;">
                <span>للتواصل مع الدعم الفني إضغط <a href="https://wa.me/966501007844">هنا</a></span>
            </li>
                  ...
                  <div class="row shortcuts px-4">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="{{url('/user/subscription')}}">
                            <span>{{__('Subscription Expires On:')}} <strong>{{\Carbon\Carbon::parse(auth()->user()->will_expire)->translatedFormat('Y-m-d')}}</strong></span>
                        </a>
                    </li>
                @endif
                ...
                <li class="nav-item">
                    ...
                    @foreach(get_option('languages',true) ?? [] as $key => $lang)
                        @php
                            $flag = $key == 'ar' ? 'sa': 'us';
                        @endphp
                        @if($key != $local || ($local == null && $key == 'ar'))
                            <a href="{{url('/local/' .$key) }}" class="nav-link"><img src="{{ asset("assets/img/flags/$flag.svg") }}" alt="{{ $lang }}" style="width: 20px;"></a>
                        @endif
                    @endforeach
                </li>
