...
'buttons' =>[
	[
	'name'=>'<i class="ni ni-calendar-grid-58"></i>&nbsp' .__('Create Schedule'),
							...
							<tbody class="tbody">
								@foreach($posts as $post)
									<tr>
										...
										<td class="text-center">{{ $post->body !=  null ? __('Plain Text') : __('Template')  }}</td>
										<td><span class="badge {{ badge($post->status)['class'] }}">{{ __($post->status) }}</span></td>
										<td>{{ Carbon\Carbon::parse($post->schedule_at)->copy()->tz($post->zone)->translatedFormat('F j, Y  g:i A')  }}</td>
										<td>
